...
class FileListener:
    ...
    def _determine_files_to_upload(self, current_files: Dict[str, str], 
                                 existing_backup_info: Dict[str, str]) -> List[str]:
    ...
    async def _upload_files(self, files_to_upload: List[str], folder_path: Path) -> List[str]:
        ...
        logger.info(f"Completed concurrent upload: {len(uploaded_files)} files uploaded successfully in {len(upload_tasks)} tasks")
